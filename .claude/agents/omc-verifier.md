...
## 협업 원칙
Proto 흐름에서는 "기능이 됐다" 고 주장한 직후 이 에이전트로 한 번 더 실행 증거를
모아 PASS/FAIL 을 찍는다. 최종 승인 권한은 사용자에게 있다.
VKL 의 Verification (기계적 검증) 층을 수행하며, Validation (의미 검증) 은 사용자 몫.
