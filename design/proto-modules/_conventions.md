...
>
> **일부 HR 은 장르 의존** — 예를 들어 `HR-3` (PCG 레이아웃), `HR-5` (유사턴제
> 불변식), `HR-6` (CombatScheme) 은 특정 장르 기반 가정이 있어 다른 장르 프로토에는
> 적용되지 않을 수 있다. 장르별 적용/미적용은 `_conventions-<genre>.md` 에
> 명시한다 (td/rail-shooter 참조).
> **범용으로 언제나 적용되는 HR**: HR-1, HR-2, HR-4, HR-7, HR-8, HR-9, HR-10,
> HR-11, HR-12, 그리고 이 문서 후반의 파일/폴더·네이밍·asmdef·검증 공통·메타 규칙.
...
### HR-5. 유사턴제 불변식 *(장르 의존 — 유사턴제 액션 전제)*
...
### HR-6. 액션 입력은 CombatScheme 을 통해서만 *(장르 의존 — 플레이어 조작 전투 전제)*
...
FT-01 (Spec Gap) + FT-02 (Hidden Semantic Rule Misfill) 연쇄 위험.
(출처: PROP-2026-04-24-002 Approved.)

### GUIDE-PCG-02. 시가지 계열 PCG 다양성 축 우선순위 (장르 한정)

시가지·도시·마을 계열 PCG 가 "다양성 부족" 으로 OR-08 REJECTED 를 받은 경우,
해결책은 다음 순서로 **영역 구조 레벨부터** 탐색한다:

1. 영역 구조 (도로 위계·블록 간격·끊긴 길·광장)
2. 클래스 분포 (블록 타입, 건물 용도)
3. 크기·높이 분포 (power curve 등)
4. 회전·색상 jitter (**선택적**, 도메인 룰 위반 없을 때만)

1·2 를 건너뛰고 바로 3·4 (특히 건물 yaw jitter) 로 가는 것은 **FT-02 혐의**.
시가지 전제에서 건물 매스는 도로 축에 정렬되는 것이 도메인 관행이다.
(출처: PROP-2026-04-24-001 Approved. 예외: 지진·무너진 도시 등 특수 연출.)
