...
- **목표**: 수강생이 `/proto-start` 에서 한 문장 아이디어를 말하면, Claude 가 장르를
...
- 공유 `hud-kit` / `sound-kit` / `save-kit` 이 작성됐지만, 장르별 특수 HUD 위젯들이
  이 공유 모듈을 어떻게 확장하는지 구체 예시 미작성 (문서상 "IHudWidget 구현" 만 언급)
